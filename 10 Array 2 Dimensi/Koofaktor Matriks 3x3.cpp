...
int main()
{
    judul("Array 2 Dimensi", "Mencari koofakor Matriks 3x3", "1.0");
    cout << "Menentukan Koofaktor Matriks Ordo 3x3" << endl;
    double matriks[3][3], koofaktor_matriks[3][3];
    char pilihan;
input_matriks:
...
cek_input_matriks:
    ...
    for (int j = 0; j < 3; j++)
        for (int k = 0; k < 3; k++)
            // rumus laplace
            koofaktor_matriks[j][k] = matriks[(j + 1) % 3][(k + 1) % 3] * matriks[(j + 2) % 3][(k + 2) % 3] - matriks[(j + 1) % 3][(k + 2) % 3] * matriks[(j + 2) % 3][(k + 1) % 3];
    cout << "-==Hasil matriks koofaktor==- " << endl;

    for (int j = 0; j < 3; j++)
    {
        for (int k = 0; k < 3; k++)
            cout << koofaktor_matriks[j][k] << " ";
        cout << endl;
    }
cek_selesai:
